  ...
  if (selectedGroup) {
    return (
      <div className="h-full overflow-y-auto p-8">
        <button
          onClick={() => { setSelectedGroup(null); setGroupContacts([]) }}
        ...
        <div className="flex items-center gap-4 mb-8">
          ...
          <div className="flex-1">
            <h1 className="text-2xl font-bold text-zinc-100">{selectedGroup.name}</h1>
            {selectedGroup.description && (
              <p className="text-sm text-zinc-500 mt-0.5">{selectedGroup.description}</p>
            )}
          </div>
        ...
        {groupContacts.length === 0 ? (
          <div className="border border-zinc-800/40 rounded-xl p-12 text-center">
            <p className="text-sm text-zinc-600">No contacts in this group yet. Add contacts from the contact detail page.</p>
          </div>
        ) : (
    ...
    <div className="h-full overflow-y-auto p-8">
      ...
      {showCreate && (
        <div className="border border-violet-500/20 rounded-xl p-5 mb-6 bg-violet-500/5">
          <h3 className="text-xs font-medium text-violet-400 uppercase tracking-wider mb-3">New Group</h3>
      ...
      {groups.length === 0 && !showCreate ? (
        <div className="border border-zinc-800/60 rounded-xl p-12 text-center">
          ...
          <p className="text-sm text-zinc-500">No groups yet. Create one to organize your contacts.</p>
        </div>
      ) : (
        <div className="border border-zinc-800/60 rounded-xl overflow-hidden divide-y divide-zinc-800/40">
          {groups.map(group => (
