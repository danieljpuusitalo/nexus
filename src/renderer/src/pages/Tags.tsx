...
export default function Tags() {
  ...
  async function handleSaveEdit() {
    ...
    setEditingTag(null)
    await loadTags()
    if (selectedTag?.id === editingTag) {
      setSelectedTag({ ...selectedTag, name: editName.trim(), color: editColor })
    }
  }
  ...
  if (selectedTag) {
    return (
      <div className="h-full overflow-y-auto p-8">
        <button
          onClick={() => { setSelectedTag(null); setTagContacts([]) }}
        ...
        <div className="flex items-center gap-4 mb-8">
          ...
          <div className="flex-1">
            <h1 className="text-2xl font-bold text-zinc-100">{selectedTag.name}</h1>
          </div>
          <span className="text-sm text-zinc-500">
            {tagContacts.length} {tagContacts.length === 1 ? 'contact' : 'contacts'}
          </span>
        </div>

        {tagContacts.length === 0 ? (
          <div className="border border-zinc-800/40 rounded-xl p-12 text-center">
            <p className="text-sm text-zinc-600">No contacts with this tag. Assign it from a contact's detail page.</p>
          </div>
        ) : (
          <div className="border border-zinc-800/60 rounded-xl overflow-hidden divide-y divide-zinc-800/40">
            {tagContacts.map(c => (
              <div key={c.id} className="flex items-center gap-4 px-5 py-3.5">
                <div className="w-9 h-9 rounded-full bg-zinc-800 flex items-center justify-center flex-shrink-0 text-sm font-semibold text-zinc-400">
                  {c.first_name[0]}{c.last_name?.[0] || ''}
                </div>
                <div className="flex-1 min-w-0">
                  <span className="text-sm font-medium text-zinc-200">{c.first_name} {c.last_name}</span>
                  {(c.job_title || c.company) && (
                    <p className="text-xs text-zinc-500 mt-0.5">
                      {c.job_title}{c.job_title && c.company ? ' at ' : ''}{c.company}
                    </p>
                  )}
                </div>
                {c.email && <span className="text-xs text-zinc-600 truncate max-w-[200px]">{c.email}</span>}
              </div>
            ))}
          </div>
        )}
    ...
    <div className="h-full overflow-y-auto p-8">
      ...
      {showCreate && (
        <div className="border border-violet-500/20 rounded-xl p-5 mb-6 bg-violet-500/5">
          <h3 className="text-xs font-medium text-violet-400 uppercase tracking-wider mb-3">New Tag</h3>
          <div className="space-y-3">
            <input
              ...
              className="w-full bg-zinc-900 border border-zinc-700/50 rounded-lg px-3 py-2 text-sm text-zinc-200 outline-none focus:border-violet-500/50"
            />
            <div>
      ...
      {tags.length === 0 && !showCreate ? (
        <div className="border border-zinc-800/60 rounded-xl p-12 text-center">
          ...
          <p className="text-sm text-zinc-500">No tags yet. Create one to label your contacts.</p>
        </div>
      ) : (
        <div className="border border-zinc-800/60 rounded-xl overflow-hidden divide-y divide-zinc-800/40">
          {tags.map(tag => (
              ...
              {editingTag === tag.id ? (
                <div className="flex items-center gap-3 flex-1">
                  <input
                    ...
                    className="bg-zinc-900 border border-zinc-700/50 rounded-lg px-3 py-1.5 text-sm text-zinc-200 outline-none focus:border-violet-500/50 w-48"
                  />
                  <div className="flex gap-1">
                    {TAG_COLORS.map(c => (
                      <button
                        ...
                        className={`w-5 h-5 rounded-full transition-all ${editColor === c ? 'ring-2 ring-white/30 scale-110' : ''}`}
                        style={{ backgroundColor: c }}
                      />
                    ))}
                  </div>
                  <button onClick={handleSaveEdit} className="px-2 py-1 text-xs text-violet-400 hover:text-violet-300 font-medium">Save</button>
                  <button onClick={() => setEditingTag(null)} className="px-2 py-1 text-xs text-zinc-500 hover:text-zinc-300">Cancel</button>
                </div>
              ) : (
                <>
                  <button
                  ...
                  <button
                    ...
                    className="text-zinc-700 hover:text-zinc-400 opacity-0 group-hover:opacity-100 transition-all text-xs flex-shrink-0"
                  >
                    Edit
                  </button>
                  {deleteConfirm === tag.id ? (
                    <div className="flex items-center gap-1.5 flex-shrink-0">
                      <button
                        ...
                        className="px-2 py-1 text-xs text-red-400 hover:text-white hover:bg-red-600 rounded transition-colors"
                      >
                        Delete
                      </button>
                      <button
